...
const readDiagram = (file_path)=>{
	return fs.promises.readFile(file_path,'utf8').then((data)=>{
		return drawionode.parseString(data).then((data_value)=>{
				...
				return drawionode.parseString(drawionode.decompress(compressed))
			})
		})
}
...
const getCollapsibleFromMxCell = (result_decompressed,width_big=null,height_big=null)=>{
	let shape = getShape(result_decompressed)
	//console.log('shape',shape)
	let style = shape.mxCell.$.style
	//console.log(style)
	let basics_lib = getBasics()
